...
from Binance.consts_enums import (
    WEBSOCKET_BASEURL_SPOT,
    ENDPOINT_EXCHANGE_INFORMATION_SPOT,
    ENDPOINT_CANDLESTICK_DATA_SPOT,
...
async def exchange_info() -> tuple:
    ...
    async with aiohttp.ClientSession() as client_session:
        ...
        status, headers, data = await http_request(
            ...
            endpoint=ENDPOINT_EXCHANGE_INFORMATION_SPOT
        )
...
async def get_klines(
    ...
    status, headers, data = await http_request(
        ...
        endpoint=ENDPOINT_CANDLESTICK_DATA_SPOT,
        params=params
    )
...
async def candlestick_stream(
    ...
    websocket_resource_url = f"{WEBSOCKET_BASEURL_SPOT}/stream?streams=btcusdt@kline_1m/" \
